...
impl SingleDirs {
    ...
    /// Generated by `single provider sync-pool` from
    /// `single_core::free_pool::FREE_PROVIDERS` — per-provider
    /// `enabled`/`disabled_reason` state (E28 spec §5.3/§17). Distinct
    /// from `providers.toml`, which carries the `single-<id>` preset
    /// metadata but has no `enabled` concept of its own.
    pub fn free_pool_registry_file(&self) -> PathBuf {
        self.root.join("free-pool.toml")
    }
