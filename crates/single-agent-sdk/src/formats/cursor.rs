//! Reads/writes `~/.cursor/mcp.json`'s `mcpServers` key. Format confirmed
//! by direct inspection of a real file on the reference machine: a
//! top-level JSON object with a `mcpServers` map of `{ command, args, env }`
//! — unlike Claude's `~/.claude.json`, there is no `"type"` field in any
//! real entry, so this writer doesn't add one.
...
pub fn apply(path: &Path, servers: &[McpServerSpec]) -> Result<Value> {
    ...
    let root_obj = root.as_object_mut().context("~/.cursor/mcp.json root is not a JSON object")?;
...
mod tests {
    ...
    #[test]
    fn adds_server_without_a_type_field() {
        ...
        assert!(result["mcpServers"]["git"].get("type").is_none());
    }
